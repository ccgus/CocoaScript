#import "COSDatabase.h"
#import "unistd.h"
#import <objc/runtime.h>

@interface COSDatabase ()

- (COSResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;

@end

@implementation COSDatabase
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    COSDatabase *self = (__bridge COSDatabase*)f;
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        COSResultSet *rs = (COSResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(COSResultSet *)resultSet {
...
- (COSStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(COSStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(COSStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    COSResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"COSDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];    
}
...
- (COSResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
...
- (COSResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    COSStatement *statement  = 0x00;
    COSResultSet *rs         = 0x00;
    ...
    if (!statement) {
        statement = [[COSStatement alloc] init];
    ...
    rs = [COSResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (COSResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (COSResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (COSResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (COSResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[COSStatement alloc] init];
